...
- **File**: `apps/intercom/src/index.ts:67-89` (registerGroup function)
- **File**: `apps/intercom/src/db.ts:686-703` (setRegisteredGroup -- SQLite only)
- **File**: `apps/intercom/src/ipc.ts:400-431` (register_group IPC handler)
...
- **File**: `apps/intercom/src/index.ts:260-266` (handleModel)
...
- **File**: `apps/intercom/rust/intercomd/src/main.rs:879-897` (apply_command_effects SwitchModel)
...
- **File**: `apps/intercom/rust/intercomd/src/telegram.rs:209-273` (route_ingress)
- **File**: `apps/intercom/rust/intercomd/src/telegram.rs:659-666` (open_sqlite)
...
- **File**: `apps/intercom/src/ipc.ts:303-314` (schedule_task handler)
- **File**: `apps/intercom/src/db.ts:464-483` (createTask -- SQLite only)
...
- **File**: `apps/intercom/src/ipc.ts:322-374` (pause_task, resume_task, cancel_task handlers)
- **File**: `apps/intercom/src/db.ts:506-551` (updateTask, deleteTask -- SQLite only)
...
- **File**: `apps/intercom/src/pg-writer.ts:57-60` (writeToOutbox type signature)
- **File**: `apps/intercom/rust/intercomd/src/outbox.rs:117-174` (drain loop match arms)
...
- **File**: `apps/intercom/src/db.ts:621-625` (setSession -- SQLite only)
- **File**: `apps/intercom/src/index.ts:128-151` (clearGroupSession -- SQLite only)
...
- **File**: `apps/intercom/rust/intercomd/src/db.rs:524-536` (set_registered_group HTTP handler)
- **File**: `apps/intercom/rust/intercomd/src/main.rs:494` (db_routes use Option<PgPool> state, not AppState)
...
- **File**: `apps/intercom/src/db.ts:105-150` (SQLite schema + migrations)
- **File**: `apps/intercom/rust/intercom-core/src/persistence.rs:348-358` (Postgres schema)
...
- **File**: `apps/intercom/rust/intercomd/src/process_group.rs:181-185` (container_config deserialization)
- **File**: `apps/intercom/rust/intercom-core/src/persistence.rs:1483` (container_config from Postgres JSONB)
...
- **Scenario**: Main group registers a project group with `containerConfig: { additionalMounts: [{ hostPath: "~/projects/Foo", readonly: false }] }`. SQLite stores it. Rust orchestrator reads from Postgres, finds `container_config = NULL`. Container spawns without the mount. The agent cannot access the project directory.
