...
*Research synthesis: NASA TRL, DoD MRL, developmental biology, complex adaptive systems theory, reliability engineering, and system readiness levels — mapped onto Demarch at v0.6.228.*

---

## 1. NASA Technology Readiness Levels (TRL 1-9) Mapped to Demarch
...
| TRL | NASA Definition | Demarch Analog |
|-----|----------------|----------------|
...
- External user validation on non-Demarch codebases
...
The most dangerous misapplication: declaring TRL 8-9 based on depth of testing in one environment (Demarch building Demarch) rather than breadth across many. Self-building is necessary but not sufficient. It is the equivalent of testing a satellite in a vacuum chamber that perfectly replicates one specific orbit.
...
### Mapping MRL Threads to Demarch

| MRL Thread | Hardware Domain | Demarch Domain | Current State |
|-----------|----------------|----------------|---------------|
...
| Signal | Pre-Percolation (Disconnected Clusters) | Post-Percolation (Giant Component) | Demarch Status |
|--------|------------------------------------------|---------------------------------------|----------------|
...
Demarch's flywheel (authority -> actions -> evidence -> authority) is an autocatalytic set by design: each component's output is another component's input. But the critical question is whether this set is *actually* self-sustaining or only *designed to be* self-sustaining.
...
Drawing the most predictive indicators from each domain, here is a three-level maturity rubric for Demarch.
...
3. **External codebase validation.** At least 3 non-Demarch codebases have been built or maintained using the platform with measured outcomes. Sprint completion rates, cost, and defect rates are comparable (within 2x) to self-building metrics.
...
| **Demarch status** | **Yes** — the system can be deployed and produces useful output | **Not yet** — the learning loop is not closed, SPC is absent, external validation is pending |

**Demarch at v0.6.228 is ready to deploy. It is not yet ready to be relied upon.**
