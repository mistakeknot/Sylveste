...
- `os/Skaffen/internal/tool/builtin.go` — registration site; `RegisterQualityHistory` is the pattern to follow
- `os/Skaffen/internal/tool/quality_history.go` — precedent for tool wrapping a store via interface
- `os/Skaffen/internal/mutations/store.go` — precedent for domain store living outside `tool/`
- `os/Skaffen/internal/evidence/emitter.go` — existing pipeline; `EmitExperiment` must not fork this
- `os/Skaffen/internal/agent/deps.go` — `Evidence` struct; correct place to add `ExperimentEvent` field
- `os/Skaffen/internal/git/git.go` — existing git ops; `GitOps` should compose this, not duplicate it
- `os/Skaffen/internal/tui/status.go` — `updateStatusSlots` signature; no `tool` import, keep it that way
- `docs/plans/2026-03-15-autoresearch-skaffen.md` — plan under review
