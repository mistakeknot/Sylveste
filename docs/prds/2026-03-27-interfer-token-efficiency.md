...
# PRD: interfer Token Efficiency & Game Playtesting

## Problem

Clavain sprints consume significant cloud API tokens. Research agents (~30-40% of volume) and C1/C2 tasks (~20-25%) are high-volume, low-stakes work that doesn't need frontier models. Meanwhile, Shadow Work playtesting runs $2-5/session in API costs, limiting continuous testing. interfer has a working server with benchmarked models but Track B5 isn't being used yet.
...
### Feature 2: sw-agent ↔ interfer Bridge for Game Playtesting

Thin adapter connecting Shadow Work's sw-agent HTTP API to interfer for zero-cost playtesting.

**Acceptance criteria:**
- [ ] Bridge reads game state from sw-agent (localhost:8790)
- [ ] Formats state as prompts for interfer (/v1/chat/completions)
...
- Modifying interfer's inference engine or experiment hooks
...
- interfer server running on localhost:8421
...
Python script that loops: read state → prompt → infer → act. Uses interfer's OpenAI-compatible API.
