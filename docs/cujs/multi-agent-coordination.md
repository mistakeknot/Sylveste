...
Before starting multi-agent work, Intercore must be running. The developer verifies with `/clavain:doctor` (which checks Intercore health among other infrastructure) or starts it with `ic serve` if needed. Multi-agent dispatch refuses to start if Intercore is unavailable.

The developer starts three agent sessions via Mycroft or manually. When agents are dispatched by Clavain or Mycroft, they auto-join Interlock on startup — no manual step required. For ad-hoc sessions, the developer runs `/interlock:join` to register the agent explicitly. Either way, Interlock creates a coordination identity. When an agent starts editing `selector.go`, it reserves the file: `reserve_files(["internal/mycroft/scheduler/selector.go"])`. Other agents querying that file see it's held and work on something else.
...
|--------|------|-----------|
| No merge conflicts when 3+ agents work in parallel | measurable | Zero git conflicts during coordinated multi-agent session (Interlock file reservation is the prevention mechanism) |
...
| Stale reservations auto-expire | measurable | Reservations from dead agents expire within adaptive timeout (default 15 min TTL, auto-renewing on edit) |
...
- **Coordination requires Intercore** — Intercore is a hard dependency for multi-agent work. If the Intercore bus isn't running, multi-agent dispatch refuses to start. Startup: `ic serve`; verification: `/clavain:doctor`.
