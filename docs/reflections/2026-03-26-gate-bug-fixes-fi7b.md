...
1. **Close child beads when parent work ships.** Batch 1 was a dependency of the calibration plan (Sylveste-0rgc). When Batch 1 shipped in the same session as Batches 2-6, only the parent's phase advanced — the child bead was orphaned as open/P0.
...
3. **Triage should check git history for open beads with code references.** `git log --grep=Sylveste-fi7b` would have immediately surfaced commit `d27b3fc`. A bd doctor check for "open beads referenced in commit messages" would catch this class of orphan.
