...
stage: design
---
# interfere: Local Inference Plugin — Implementation Plan
...
- An OpenAI-compatible client can connect to interfere and stream completions
...
- `interverse/interfere/server/main.py` — Starlette app with `/v1/chat/completions`, `/health`
- `interverse/interfere/server/inference.py` — Metal subprocess, `generate_step` loop
- `interverse/interfere/server/queue.py` — `PriorityRequestQueue` class
- `interverse/interfere/server/models.py` — model loading, memory management, hot-swap
- `interverse/interfere/server/thermal.py` — thermal monitoring via notify API
- `interverse/interfere/.claude-plugin/plugin.json` — plugin manifest
- `os/Clavain/config/routing.yaml` — Track B5 local_models section
- `os/Clavain/scripts/lib-routing.sh` — local model tier mappings

**Key Links:**
- Clavain `routing_resolve_model()` -> Track B5 -> interfere `/v1/chat/completions`
...
**Files:**
- Create: `interverse/interfere/.claude-plugin/plugin.json`
- Create: `interverse/interfere/CLAUDE.md`
- Create: `interverse/interfere/AGENTS.md`
- Create: `interverse/interfere/pyproject.toml`
- Create: `interverse/interfere/server/__init__.py`
...
```python
# interverse/interfere/server/__init__.py
"""interfere: Local MLX-LM inference server."""
```

**Step 5: Commit**
```bash
git add interverse/interfere/
git commit -m "feat(interfere): scaffold plugin directory and manifest"
```

<verify>
- run: `python -c "import json; d=json.load(open('interverse/interfere/.claude-plugin/plugin.json')); assert d['name']=='interfere'"`
  expect: exit 0
- run: `ls interverse/interfere/server/__init__.py`
  expect: exit 0
</verify>
...
**Files:**
- Create: `interverse/interfere/server/metal_worker.py`
- Create: `interverse/interfere/tests/test_metal_worker.py`

**Step 1: Write the failing test**
```python
# interverse/interfere/tests/test_metal_worker.py
import multiprocessing
import pytest

def test_metal_worker_starts_and_responds_to_health():
    """Worker subprocess starts, reports health, and shuts down cleanly."""
    from interfere.server.metal_worker import MetalWorker
...
Run: `cd interverse/interfere && uv run pytest tests/test_metal_worker.py -v`
Expected: FAIL with ImportError

**Step 3: Write minimal implementation**
```python
# interverse/interfere/server/metal_worker.py
...
**Step 5: Commit**
```bash
git add interverse/interfere/server/metal_worker.py interverse/interfere/tests/
git commit -m "feat(interfere): Metal subprocess with memory safety and health check"
```
...
**Files:**
- Create: `interverse/interfere/server/queue.py`
- Create: `interverse/interfere/tests/test_queue.py`

**Step 1: Write the failing test**
```python
# interverse/interfere/tests/test_queue.py
import asyncio
import pytest
import pytest_asyncio

from interfere.server.queue import PriorityRequestQueue, InferenceRequest
...
**Step 2: Run test to verify it fails**
Run: `cd interverse/interfere && uv run pytest tests/test_queue.py -v`
Expected: FAIL with ImportError

**Step 3: Write minimal implementation**
```python
# interverse/interfere/server/queue.py
...
**Step 5: Commit**
```bash
git add interverse/interfere/server/queue.py interverse/interfere/tests/test_queue.py
git commit -m "feat(interfere): priority request queue with backpressure"
```
...
**Files:**
- Create: `interverse/interfere/server/main.py`
- Create: `interverse/interfere/server/schema.py`
- Create: `interverse/interfere/tests/test_server.py`

**Step 1: Write the failing test**
```python
# interverse/interfere/tests/test_server.py
import pytest
from httpx import AsyncClient, ASGITransport
from interfere.server.main import create_app
...
**Step 2: Run test to verify it fails**
Run: `cd interverse/interfere && uv run pytest tests/test_server.py -v`
Expected: FAIL with ImportError

**Step 3: Write schema module**
```python
# interverse/interfere/server/schema.py
...
```python
# interverse/interfere/server/main.py
    ...
    async def chat_completions(request: Request) -> StreamingResponse | JSONResponse:
        ...
        if req.stream:
            ...
            async def event_stream():
                if dry_run:
                    for token in ["Hello", " from", " interfere", "!"]:
...
**Step 6: Commit**
```bash
git add interverse/interfere/server/main.py interverse/interfere/server/schema.py interverse/interfere/tests/test_server.py
git commit -m "feat(interfere): Starlette HTTP server with OpenAI-compatible streaming"
```
...
**Files:**
- Create: `interverse/interfere/server/models.py`
- Create: `interverse/interfere/tests/test_models.py`

**Step 1: Write the failing test**
```python
# interverse/interfere/tests/test_models.py
import pytest
from interfere.server.models import ModelRegistry
...
**Step 2: Run test to verify it fails**
Run: `cd interverse/interfere && uv run pytest tests/test_models.py -v`
Expected: FAIL

**Step 3: Write implementation**
```python
# interverse/interfere/server/models.py
...
**Step 5: Commit**
```bash
git add interverse/interfere/server/models.py interverse/interfere/tests/test_models.py
git commit -m "feat(interfere): model registry with memory budget enforcement"
```
...
**Files:**
- Create: `interverse/interfere/server/inference.py`
- Create: `interverse/interfere/tests/test_inference.py`
...
```python
# interverse/interfere/server/inference.py
...
Run: `cd interverse/interfere && uv run pytest tests/test_inference.py -v -s`
Expected: PASS (downloads tiny model on first run)

**Step 4: Commit**
```bash
git add interverse/interfere/server/inference.py interverse/interfere/tests/test_inference.py
git commit -m "feat(interfere): MLX inference engine with generate_step loop"
```
...
**Files:**
- Modify: `interverse/interfere/server/metal_worker.py` — add GENERATE command
- Modify: `interverse/interfere/server/main.py` — connect to worker
- Create: `interverse/interfere/tests/test_e2e.py`

**Step 1: Write the failing E2E test**
```python
# interverse/interfere/tests/test_e2e.py
import pytest
from httpx import AsyncClient, ASGITransport
...
**Step 2: Run test**
Run: `cd interverse/interfere && uv run pytest tests/test_e2e.py -v`
Expected: PASS (dry_run mode, no Metal needed)

**Step 3: Commit**
```bash
git add interverse/interfere/
git commit -m "feat(interfere): end-to-end streaming verified in dry-run mode"
```

<verify>
- run: `cd interverse/interfere && uv run pytest tests/ -v`
  expect: exit 0
</verify>
...
```yaml
# Track B5: Local model routing (interfere)
local_models:
...
git commit -m "feat(clavain): Track B5 local model routing via interfere"
```
...
**Files:**
- Create: `interverse/interfere/server/thermal.py`
- Create: `interverse/interfere/tests/test_thermal.py`

**Step 1: Write the failing test**
```python
# interverse/interfere/tests/test_thermal.py
import sys
import pytest

@pytest.mark.skipif(sys.platform != "darwin", reason="macOS only")
def test_thermal_monitor_reads_pressure():
    from interfere.server.thermal import ThermalMonitor
...
**Step 2: Write implementation**
```python
# interverse/interfere/server/thermal.py
...
**Step 3: Run test**
Run: `cd interverse/interfere && uv run pytest tests/test_thermal.py -v`
Expected: PASS on macOS

**Step 4: Commit**
```bash
git add interverse/interfere/server/thermal.py interverse/interfere/tests/test_thermal.py
git commit -m "feat(interfere): thermal monitoring via macOS notify API (no sudo)"
```
...
**Files:**
- Create: `interverse/interfere/server/experiments/early_exit.py`
- Create: `interverse/interfere/tests/test_early_exit.py`

**Step 1: Write the failing test**
```python
# interverse/interfere/tests/test_early_exit.py
import pytest

def test_early_exit_hook_computes_confidence():
    pytest.importorskip("mlx")
    import mlx.core as mx
    from interfere.server.experiments.early_exit import EarlyExitHook
...
**Step 2: Write implementation**
```python
# interverse/interfere/server/experiments/__init__.py
"""Experiment hooks for interfere inference pipeline."""

# interverse/interfere/server/experiments/early_exit.py
...
**Step 3: Run tests**
Run: `cd interverse/interfere && uv run pytest tests/test_early_exit.py -v`
Expected: PASS

**Step 4: Commit**
```bash
git add interverse/interfere/server/experiments/ interverse/interfere/tests/test_early_exit.py
git commit -m "feat(interfere): early exit experiment hook with confidence check"
```
...
**Files:**
- Create: `interverse/interfere/server/experiments/reservoir_routing.py`
- Create: `interverse/interfere/tests/test_reservoir_routing.py`

**Step 1: Write the failing test**
```python
# interverse/interfere/tests/test_reservoir_routing.py
import pytest

def test_reservoir_readout_classifies():
    pytest.importorskip("mlx")
    import mlx.core as mx
    from interfere.server.experiments.reservoir_routing import ReservoirReadout
...
**Step 2: Write implementation**
```python
# interverse/interfere/server/experiments/reservoir_routing.py
...
**Step 3: Run tests**
Run: `cd interverse/interfere && uv run pytest tests/test_reservoir_routing.py -v`
Expected: PASS

**Step 4: Commit**
```bash
git add interverse/interfere/server/experiments/reservoir_routing.py interverse/interfere/tests/test_reservoir_routing.py
git commit -m "feat(interfere): reservoir routing readout MLP for task classification"
```
...
**Files:**
- Create: `interverse/interfere/AGENTS.md`
- Modify: `interverse/interfere/CLAUDE.md` — add experiment docs
- Modify: `interverse/interfere/.claude-plugin/plugin.json` — add MCP server
...
```json
{
  "mcpServers": {
    "interfere": {
      "type": "stdio",
      "command": "uv",
      "args": ["run", "python", "-m", "interfere.mcp"],
...
```bash
git add interverse/interfere/
git commit -m "docs(interfere): AGENTS.md, MCP server config, experiment documentation"
```

<verify>
- run: `cd interverse/interfere && uv run pytest tests/ -v`
  expect: exit 0
- run: `python -c "import json; d=json.load(open('interverse/interfere/.claude-plugin/plugin.json')); assert 'mcpServers' in d"`
  expect: exit 0
</verify>
