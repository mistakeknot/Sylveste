---
title: "interfer Token Efficiency & Game Playtesting"
date: 2026-03-27
status: reviewed
bead: sylveste-86r
complexity: C3
...
# Plan: interfer Token Efficiency & Game Playtesting

## Overview

Use interfer's existing server to:
1. Route Clavain tasks to local models via Track B5 (shadow → enforce)
2. Build a playtest bridge connecting Shadow Work's debug API to interfer
...
- **H2 (architecture):** Bridge script inside interfer couples it to Shadow Work. **Fix:** Moved to `scripts/` at monorepo root.
...
### Task 0: Wire PriorityRequestQueue into interfer server
**Files:** `interverse/interfer/server/main.py`, `interverse/interfer/server/queue.py`
...
**Files:** `scripts/playtest-bridge.py` (new, at monorepo root — NOT inside interfer)
**Effort:** Medium

Standalone script at monorepo root. Interfere is a downstream service (it serves inference); the bridge is a consumer that also happens to consume Shadow Work's API. Placing it inside interfer would couple the inference server to a specific game's HTTP contract.

Python script that connects sw-agent HTTP API (localhost:8790) to interfer (localhost:8421):

```
Loop:
  1. Check interfer /health for thermal state (before inference, not just at loop start)
  ...
  5. Format as system prompt (with domain preamble) + user message for interfer
...
### Task 3b: Wire PromptCacheManager into interfer
**Files:** `interverse/interfer/server/main.py`
**Effort:** Small (integration of existing code)
...
Start interfer (35B model only) + Shadow Work, run `climate-cascade` campaign via playtest bridge:
...
- [ ] PriorityRequestQueue wired into interfer server (concurrent request safety)
...
- [ ] PromptCacheManager wired into interfer server
...
- Adding vision/multimodal to interfer (see investigation doc; Qwen3-VL-30B-A3B recommended for future sprint)
- Modifying interfer experiments (TurboQuant, early exit, etc.)
