...
- Promotion path: `sylvst.com/live` reads `main`, not `oyrf-data` (see `docs/deploy/sylvst-com.md`). The `ops/oyrf-cost-promote` systemd timer runs daily, shortly after the export lands, diffs new `source=interstat` rows out of `oyrf-data`, and opens a PR onto `main` with `gh pr merge --auto` set — it lands only once the required `Generator and parity checkers` check passes, never by force-push or admin bypass. Idempotent: a run with nothing new, or with a promotion PR already open, does nothing.
