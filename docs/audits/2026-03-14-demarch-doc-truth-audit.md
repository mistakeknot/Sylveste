# Demarch Documentation Truth Audit

**Date:** 2026-03-14  
**Scope:** Demarch root plus repos under `apps/`, `core/`, `os/`, `sdk/`, and `interverse/`  
...
1. The root canon no longer agrees on what Demarch is. After the Skaffen addition, some docs describe a five-pillar platform while others describe six pillars.
...
1. **Root architecture drift after Skaffen is now a canon-level contradiction.**  
   [docs/demarch-vision.md:238](../demarch-vision.md#L238) says Demarch has six pillars and includes Skaffen; [docs/demarch-reference.md:7](../demarch-reference.md#L7) also defines a six-pillar model. But [docs/architecture.md:7](../architecture.md#L7), [docs/glossary.md:7](../glossary.md#L7), and [README.md:66](../../README.md#L66) still describe Demarch as a five-pillar system. Repo reality supports the six-pillar version because `os/Skaffen` now exists as a first-level repo.

2. **The root canon has broken case-sensitive links to major pillar docs.**  
   [docs/architecture.md:135](../architecture.md#L135) and [docs/architecture.md:136](../architecture.md#L136) link to `os/clavain/...` and `apps/autarch/...`; those paths do not exist. The same lowercase drift appears in [docs/demarch-vision.md:249](../demarch-vision.md#L249) and [docs/demarch-vision.md:299](../demarch-vision.md#L299). These links only work if readers mentally correct the path casing.

3. **The root roadmap is stale both structurally and factually.**  
   [docs/demarch-roadmap.md:3](../demarch-roadmap.md#L3) still leaves module and bead totals as unevaluated shell commands instead of current values. The ecosystem table uses lowercase paths like [docs/demarch-roadmap.md:69](../demarch-roadmap.md#L69) and [docs/demarch-roadmap.md:70](../demarch-roadmap.md#L70), and it omits real top-level repos `interverse/interhelm`, `interverse/interlab`, and `sdk/interbase`. Live repo inventory is 64 first-level repos in those pillars, while live bead counts from `bd status --json` show 736 open issues.

4. **Kernel gate enforcement is overstated relative to the documented phase mapping.**  
   [docs/demarch-vision.md:125](../demarch-vision.md#L125) calls gates “kernel-enforced invariants,” but [docs/glossary.md:88](../glossary.md#L88) documents that OS-created sprints only get kernel gate coverage on 5 of 8 transitions because `plan-reviewed` and `shipping` diverge from kernel expectations. [docs/architecture.md:111](../architecture.md#L111) then describes the lifecycle as if the kernel simply walks the chain. The current wording hides a real enforcement gap.
...
1. Normalize the root model first: decide whether Demarch is formally five pillars or six, then align `README.md`, `docs/architecture.md`, `docs/glossary.md`, `docs/demarch-reference.md`, and `docs/demarch-vision.md`.
